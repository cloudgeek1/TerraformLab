...
module "location_uk1w" {
  source = "./location"

  web_server_location      = "westeurope"
  web_server_rg            = "${var.web_server_rg}-uk1w"
  resource_prefix          = "${var.resource_prefix}-uk1w"
...
resource "azurerm_traffic_manager_endpoint" "traffic_manager_uk1w" {
  name                   = "${var.resource_prefix}-uk1w-endpoint"
  resource_group_name    = "${module.location_us2w.web_server_rg_name}"
  profile_name           = "${azurerm_traffic_manager_profile.traffic_manager.name}"
  target_resource_id     = "${module.location_uk1w.web_server_lb_public_ip_id}"
  type                   = "azureEndpoints"
  weight                 = 100
}
...
resource "azurerm_virtual_network_peering" "jump_server_peer_web_uk1w" {
  name                         = "jump-uk1w-peer-web-uk1w"
  resource_group_name          = "${azurerm_resource_group.jump_server_rg.name}"  
  virtual_network_name         = "${azurerm_virtual_network.jump_server_vnet.name}"  
  remote_virtual_network_id    = "${module.location_uk1w.web_server_vnet_id}"
  allow_virtual_network_access = true
  depends_on                   = ["azurerm_subnet.jump_server_subnet"]
}

resource "azurerm_virtual_network_peering" "web_uk1w_peer_jump_server" {
  name                         = "web-uk1w-peer-jump-uk1w"
  resource_group_name          = "${module.location_uk1w.web_server_rg_name}"  
  virtual_network_name         = "${module.location_uk1w.web_server_vnet_name}"  
  remote_virtual_network_id    = "${azurerm_virtual_network.jump_server_vnet.id}"
  allow_virtual_network_access = true
  depends_on                   = ["azurerm_subnet.jump_server_subnet"]
}
