...
resource "azurerm_resource_group" "web_server_rg" {
  ...
  location = "${var.web_server_location_eu}"
...
resource "azurerm_virtual_network" "web_server_vnet" {
  ...
  address_space       = ["${var.web_server_address_space_eu}"]
}

resource "azurerm_subnet" "web_server_subnet" {
  name                      = "${var.resource_prefix}-${substr(var.web_server_subnets_eu[count.index], 0, length(var.web_server_subnets_eu[count.index]) - 3)}-subnet"
  resource_group_name       = "${azurerm_resource_group.web_server_rg.name}"
  virtual_network_name      = "${azurerm_virtual_network.web_server_vnet.name}"
  address_prefix            = "${var.web_server_subnets_eu[count.index]}"
  network_security_group_id = "${count.index == 0 ? "${azurerm_network_security_group.web_server_nsg.id}" : ""}"
  count                     = "${length(var.web_server_subnets_eu)}"
}
...
resource "azurerm_virtual_machine_scale_set" "web_server_eu" {
  ...
  network_profile {
    ...
    ip_configuration {
      ...
      primary                                = true
      subnet_id                              = "${azurerm_subnet.web_server_subnets_eu.*.id[0]}"
